...
image_file_path = os.getcwd()+'/ssd.pytorch/data/VOCdevkit/VOC2022/JPEGImages'
    ...
    if each_yolo_file.endswith("txt"):
        ...
        if not image_name == each_yolo_file:
            ...
            with open('XML' + os.sep + each_yolo_file.replace('txt', 'xml'), 'w') as f:
                ...
                f.write('\t<path>' + image_file_path + os.sep + image_name + '</path>\n')
                f.write('\t<source>\n')
                f.write('\t\t<database>Strawberry</database>\n')
                f.write('\t</source>\n')
                f.write('\t<size>\n')
